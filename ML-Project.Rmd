...
We can now apply PCA to these features - for a threshold of 0.80, we find 12 principal components. For a threshold of 0.95, we find 25 principal components. (The threshold is the amount of variance in the data accounted for by the principle components - a higher threshold implies more principal components).
...
---------------|---------------------|--------------------|------------
LDA, PCA(0.95) | `r round(ldaPCA95$results$Accuracy, 3)` | `r round(cmLdaPCA95$overall[1], 3)`
QDA, PCA(0.95) | `r round(qdaPCA95$results$Accuracy, 3)` | `r round(cmQdaPCA95$overall[1], 3)`
RPART, PCA(0.95) | `r round(rpartPCA95$results$Accuracy[1], 3)` | `r round(cmRpartPCA95$overall[1], 3)`
TREEBAG, PCA(0.95) | `r round(bagPCA95$results$Accuracy, 3)` | `r round(cmBagPCA95$overall[1], 3)`
RF, PCA(0.80) | `r round(rfPCA80$results$Accuracy[1], 3)` | `r round(cmRfPCA80$overall[1], 3)`
RF, PCA(0.95) | `r round(rfPCA95$results$Accuracy[1], 3)` | `r round(cmRfPCA95$overall[1], 3)`
